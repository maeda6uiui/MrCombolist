...
def main(args):
    ...
    #Output each column to a gzip file
    logger.info("Writing out each column to a gzip file...")
    for input_file in input_files:
        ...
        #Output "email" column to a gzip file
        email_output_file=email_output_dir.joinpath(f"{input_file.stem}.txt.gz")
        with gzip.open(email_output_file,"wt",encoding="utf-8") as wt:
            for email in df["email"]:
                wt.write(f"{email}\n")

        #Output "poh" column to a gzip file
        poh_output_file=poh_output_dir.joinpath(f"{input_file.stem}.txt.gz")
        with gzip.open(poh_output_file,"wt",encoding="utf-8") as wt:
            for poh in df["poh"]:
                wt.write(f"{poh}\n")

    logger.info("Finished writing out each column to a gzip file")
